...
void DS3231_killSQW(){
	...
	twi_writeChar(DS3231_ADDRESS<<1|TW_WRITE);
	twi_writeChar(DS3231_REG_CON);
	twi_writeChar(0x04|alm);
	twi_STOP();
}
void enab_alm(bool a){
    alm=a;
}
...
void DS3231_getTime(uint8_t *second, uint8_t *minute,
    ...
    *year       = bcdToDec(twi_readChar_NACK());
	twi_STOP();
}

void DS3231_setAlarm(uint8_t alm_h,uint8_t alm_m){
	twi_START();
	twi_writeChar(DS3231_ADDRESS<<1|TW_WRITE);
	twi_writeChar(0x07);
	twi_writeChar(decimalToBcd(0));
	twi_writeChar(decimalToBcd(alm_m));
	twi_writeChar(decimalToBcd(alm_h));
	twi_writeChar(0x80);
	twi_STOP();  
} 
